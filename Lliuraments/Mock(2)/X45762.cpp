...
// Funcion que lee los datos de la imagen que queremos hacer
Image readImage()
{
    Image input;
    int filas, m;
    cin >> input.name >> input.depth >> input.i >> input.j >> filas >> m;
    input.v = vector<string>(filas);
...
// Funcion que calcula el valor minimo que tiene que tener el cuadro para que
// entren todas las imagenes
void computeMinimumBoardDimensions(const ListImages &listimages, int &N, int &M) {
    N = M = 0;
    for (int i = 0; i < listimages.size(); ++i) {
        if (N < listimages[i].i + listimages[i].v.size()) N = listimages[i].i + listimages[i].v.size();
        if (M < listimages[i].j + listimages[i].v[0].size()) M = listimages[i].j + listimages[i].v[0].size();
    }
}
...
// Funcion que compara dos imagen segun la profundifaf y orden lexicografico
bool compareImages(Image image1, Image image2)
{
    if (image1.depth != image2.depth) return image1.depth > image2.depth;
    else return image1.name < image2.name;
}
