/* program that calculates if a point is inside, outside or in the border of a
rectangle */
...
string containtment(Point p, Rectangle r) {
    string veredict;
    if (p.x < r.xmin or p.y < r.ymin or p.x > r.xmax or p.y > r.ymax) veredict = "outside";
    else if (p.x == r.xmin or p.y == r.ymin or p.x == r.xmax or p.y == r.ymax) veredict = "border";
    else veredict = "inside";
    return veredict;
}
